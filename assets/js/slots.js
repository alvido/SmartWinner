...
var slotMachine = {
	...
	spinURL: '/slotmachinescript/slots/spin.php', // point to the server component to call to get spin results.
	...
	init: function() {
		...
		if (slotMachine.soundEnabled)  {
			soundManager.url = "/slotmachinescript/js/";
	...
	spin: function() {
		if (window.loggedOut){
			top.location.href = 'http://www.smartwinners.co.uk/Lottery_signin.aspx';
			return false;
		}
	...
	_end_spin_after_payout: function(data) {
		...
		slotMachine.change_bet(0); // Adjust the bet down, if new balance is lower than bet
		if (slotMachine.get_balance() >= minBet) {
